...
const Home = ({ isDarkMode, activeColor, activeBg, activeBorder, activeShadow }: iHomeProps) => {
	...
	*/
	const handleDownloadPdf = () => {
		...
		{
			const options = {
			margin: [0.53, 0.63, 0.59, 0.63],
			filename: 'vatnaResume.pdf',
			image: { 
				type: 'jpeg', 
				quality: 0.98 
			},
			...
			<nav 
				...
				<div 
					...
					<div 
						...
						<div
							className={`focus-in-expand flex flex-col gap-1 text-base-clamp  text-gray-800 dark:text-gray-300`}>
							...
							<div 
								...
								<span>I’m a</span>
								<span 
									className={`text-animation text-lg-clamp ${activeColor} uppercase px-2 bg-black dark:bg-white`}>
									Front-end Developer
								</span>
							</div>
							<p 
							...
							<div 
								className="flex justify-center sm:justify-start items-center gap-x-1.5 sm:mt-0 mt-2">
								{socalMedia.map((socal, index) => (
									<Tooltip
										...
										arrow>
										<NavLink to={socal.socal_link} target="_blank" rel="noopener noreferrer">
											<button
												className={`group w-10 h-10 flex items-center justify-center rounded-xl border ${activeBorder} ${'dark:'+ activeBorder} bg-white/20 dark:bg-white/5 
														backdrop-blur-md shadow-md transition-all duration-300 ease-in-out 
														hover:scale-105 hover:shadow-xl hover:border-gray-400/60
														 dark:hover:border-gray-500/50
														focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-gray-400`}
												aria-label={socal.socal_name}>
												<img
