...
*/
const Experiences = ({ activeColor, activeBorder, activeShadow, activeHover}: iExp) => {
    /* 
        Start:Delare hook 
    */
  ...
  return (
    <>
        ...
        <div 
            ...
            <div 
                className={`w-full h-auto border ${activeBorder} bg-gray-300/20 dark:bg-gray-700/20 backdrop-blur-sm backdrop-sepia-0 rounded-xl shadow-sm ${activeShadow} px-3 sm:px-6 md:px-8 py-2 sm:py-3 md:py-4  flex flex-col items-center gap-y-3 sm:gap-y-4`}>
                <nav 
                    ...
                    <p 
                        className={`dark:text-gray-300 text-gray-700 text-wrap text-md-clamp text-justify sm:text-center leading-normal font-medium`}>
                ...
                <div 
                    className="w-full flex flex-wrap justify-center gap-x-3 lg:gap-x-4 md:gap-x-8 gap-y-2 sm:gap-y-3">
                    {
                        ...
                        <div
                            key={exp.id}
                            className={`group w-full sm:w-60 h-[130px] sm:h-[330px]  max-[506px]:h-auto border ${activeBorder} rounded-lg shadow-sm  overflow-hidden hover:shadow-xl transition-shadow duration-300 cursor-pointer animate__zoomInDown flex flex-row sm:flex-col bg-gray-300/20 dark:bg-gray-700/20 backdrop-blur-sm  ${activeShadow}`}
                            style={{ animationDuration: '1s' }}
                            onClick={() => setOpenModalId(exp.id)}>
                            <div 
                                className="w-[40%] sm:w-full h-full sm:h-[60%]  overflow-hidden">
                                <img
                                    src={exp.image}
                                    alt={exp.compenyName}
                            ...
                            <div 
                                ...
                                <div>
                                    ...
                                    <p 
                                        className={`text-xs-clamp sm:text-sm line-clamp-none sm:line-clamp-2 dark:text-gray-300 text-gray-500`}>
                                        {exp.position}
                                    </p>
                                    <p 
                                        className={`text-xs-clamp  mt-1 dark:text-gray-300 text-gray-500`}>
                                        {exp.sd_ed}
                                    </p>
                                    </div>
                                        <button 
                    ...
                    </div>
                    {
                        openModalId && (
                            <div 
                                ...
                                <div 
                                    className={`backdrop-blur-md backdrop-saturate-0 bg-gray-500/20 dark:bg-black/20 text-gray-200 sm:mx-0 mx-2 rounded-xl shadow-xl max-w-lg w-full  relative max-h-[90vh] overflow-y-auto  animate__zoomIn`}
                                    style={{ animationDuration: '1s' }}>
                                    {(() => {
                                        ...
                                        <>
                                            <div 
                                                className={`sticky top-0 z-10 px-4 sm:px-5 pt-4 border-b ${activeBorder}  bg-gray-700`}>
                                                <span 
                                                ...
                                                <p 
                                                    className={`text-gray-400 mb-4 text-sm-clamp`}>
                                                    {exp.sd_ed}
                                                </p>
                                            </div>
                                            {
                                                ...
                                                <ul 
                                                    className={`list-disc list-inside text-sm-clamp px-4 sm:px-6 space-y-1 marker:${activeColor} text-gray-300`}>
                                                    ...
                                                    {Array.isArray(exp.details) &&
                                                        ...
                                                        (exp.details as { positionName: string; details_p: string[] }[]).map((pos, index) => (
                                                            <div 
                                                                ...
                                                                <ul 
                                                                    className={`list-disc list-inside text-sm-clamp  space-y-1 ml-4 marker:${activeColor || 'text-gray-700'} text-gray-300`}>
